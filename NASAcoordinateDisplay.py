...
#give Target Latitude = 33.7209
#give Target Longitude = 73.0667

target_lat = float(input("give Target Latitude = "))
target_lon = float(input("give Target Longitude = "))

# Create a point feature using the target location
target_point = ee.Geometry.Point(target_lon, target_lat)
...
# start and end dates '2023-07-01', '2024-07-01'

wrs2_geometry = ee.Geometry.Polygon([[
  [target_lon - 0.1677, target_lat - 0.2617],
  [target_lon - 0.1677, target_lat + 0.2264],
  [target_lon + 0.3323, target_lat + 0.2264],
  [target_lon + 0.3323, target_lat - 0.2617]
]])
...
prop_dict["Target Latitude"] = target_lat
prop_dict["Target Longitude"] = target_lon
...
# Display the Landsat scene extent on a map
Map = geemap.Map()
Map.centerObject(target_point, 10)

rgbVis = {'bands':["B4","B3","B2"], min:0, max:0.3}

Map.addLayer(most_recent_landsat, rgbVis,  'Landsat Scene')
Map.addLayer(wrs2_geometry, {'color': 'red', 'opacity': 0.5}, 'Landsat Scene Extent')

Map.addLayer(target_point, {'color': 'blue'}, 'Target Point')
#Map
